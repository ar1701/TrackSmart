...
window.enhancedHandleRequest = async function(requestId, action) {
  ...
  try {
    ...
    // Make API call
    const response = await fetch(`/api/providers/requests/${requestId}/${action}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const memberElements = document.querySelectorAll('p:contains("Member Since:")');
  memberElements.forEach(el => {
    if (el.textContent.includes('Invalid Date') || el.textContent.includes('NaN')) {
      el.innerHTML = el.innerHTML.replace(/Invalid Date|NaN\/NaN\/NaN/, 'Not available');
    }
  });
