...
const dashboardPath = '/home/ayush/Desktop/GitHub Desktop/TrackSmart/views/providerPage/dashboard.ejs';
  ...
  scriptMatch.forEach((script, index) => {
    ...
    lines.forEach((line, lineNum) => {
      ...
      if (line.includes('<%') && line.includes('%>')) {
        console.log(`   Line ${lineNum + 1}: EJS template code in JS: ${line.trim()}`);
      }
    });
    
    console.log(`   Brace balance: ${braceCount === 0 ? 'OK' : 'UNBALANCED (' + braceCount + ')'}`);
...
ejsMatches.forEach((match, index) => {
  if (!match.endsWith('%>')) {
    console.log(`⚠ Unclosed EJS block ${index + 1}: ${match.substring(0, 50)}...`);
  }
});
