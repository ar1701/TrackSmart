...
async function testProviderDashboard() {
  ...
  try {
    ...
    const providerLoginExists = await page.$eval(
      "body",
      (body) =>
...
if (require("fs").existsSync("node_modules/puppeteer")) {
  ...
  console.log(
    '   fetch("/api/providers/requests/test/accept", {method:"POST"})'
  );
  ...
  const testEndpoints = async () => {
    try {
      ...
      const tests = [
        ...
        {
          name: "Accept API",
          url: "http://localhost:4000/api/providers/requests/test/accept",
          method: "POST",
        },
      ];
